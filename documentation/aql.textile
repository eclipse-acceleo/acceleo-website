...
This section describes the syntax of an AQL expression.
...
|<. _expression_ *->* _service_name_*(* ( _expression_ (*,* _expression_ ) * ) ? *)* |call on the collection itself it the expression is not a collection it will be wrapped into an ordered set| mySequence->sep(',') |
